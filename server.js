...
function startChannelProcess(id, url, streamType = 0, alwaysOn = false, group = '') {
    ...
    let hlsTime = '5';
    let hlsListSize = '6';
    // 60 ثانية: يمنح المهلة بلطف للمصادر الحية (30 ثانية سابقة كانت توقف ffmpeg
    // بسرعة على أي تأخير بسيط → إعادة تشغيل متكررة → ارتداد المشغّل لبداية الشريط)
    let rwTimeout = '60000000';
    ...
    ffmpegArgs.push(
        ...
        // بثّ VBR عالٍ (10Mbps +): نبضة GOP أو إعادة اتصال من المصدر تملأ الطابور دفعةً واحدة.
        // 1024 (كانت مخصصة لمصادر هادئة) تفيض → ffmpeg يُسقط حزم فيظهر التقطيع كأنه هبوط fps/بت ريت
        // بلا انقطاع صريح. نعيدها لـ 4096 ليتحمّل الموجات وتظل مستقرة.
